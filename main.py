...
def output(case):
    ...
    while True:
        ...
        if event == "_COPY_":
            # if isGuiInput:
            gaming_array = values["-TABLE-"]
            a = gaming_array[0]
            # print(a)
            print(data[gaming_array[0]][4])
            pyperclip.copy(data[a][4])
            sg.popup("Copied password")

            # for uid, un, email, website, pwd in data[a]:
            #    print(pwd)
            # for uid, username, email, website, pwd in data:
            #   print(pwd[a])
        # else:
        #    gaming_array = values["-TABLE-"]
        #   for uid, username, email, website, pwd in data:
        #       if uid - 1 in gaming_array:
        #           pyperclip.copy(pwd)
        #           sg.popup("Copied password")
        #           print(pwd)
        # print(event, values)
...
def creation():
    ...
    while True:
        ...
        if event == sg.WINDOW_CLOSED or event == "Cancel":
            ...
            if not username:
                sg.popup(f"A username is required")
            elif not email:
